...
def ms_ssim_loss(recon_x, x, data_range=1., weights=None, win_size=11, win_sigma=1.5, k1=0.01, k2=0.03, sigma=1.5, reduction='mean'):
    if weights is None:
        weights = torch.tensor([0.0448, 0.2856, 0.3001, 0.2363, 0.1333], dtype=x.dtype, device=x.device)
    ...
    for level in range(levels):
        ...
        mu_x = filters.gaussian_blur2d(X, (win_size, win_size), (win_sigma, win_sigma))
        mu_y = filters.gaussian_blur2d(Y, (win_size, win_size), (win_sigma, win_sigma))

        sigma_x2 = filters.gaussian_blur2d(X * X, (win_size, win_size), (win_sigma, win_sigma)) - mu_x ** 2
        sigma_y2 = filters.gaussian_blur2d(Y * Y, (win_size, win_size), (win_sigma, win_sigma)) - mu_y ** 2
        sigma_xy = filters.gaussian_blur2d(X * Y, (win_size, win_size), (win_sigma, win_sigma)) - mu_x * mu_y
    ...
    ms_ssim_val = torch.prod(torch.stack(ssim_vals) ** weights.view(-1, 1, 1, 1), dim=0)

    if reduction == 'none':
        return 1 - ms_ssim_val
    elif reduction == 'mean':
        return torch.mean(1 - ms_ssim_val)
    elif reduction == 'sum':
